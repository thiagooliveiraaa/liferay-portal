...
export function PublishedAppsDashboardPage() {
  ...
  const [showDashboardNavigation, setShowDashboardNavigation] = useState(true);
  const [dashboardNavigationItems, setDashboardNavigationItems] = useState(
    initialDashboardNavigationItems
  );
  const [page, setPage] = useState(1);
  const [publishedAppTable, setPublishedAppTable] = useState<PublishedAppTable>(
  ...
  useEffect(() => {
    const makeFetch = async () => {
      ...
      if (accountCustomField) {
        const accountCatalogId = Number(accountCustomField.customValue.data);
        ...
        const membersList = accountsListResponse?.items.map(
          (member: UserAccountProps) => {
            return {
              ...
              name: member.name,
              role: getRolesList(member.accountBriefs, selectedAccount.id),
              userId: member.id,
            } as MemberProps;
          }
        );
    ...
    <div className="published-apps-dashboard-page-container">
      ...
      {!loading && selectedNavigationItem === "Apps" && (
        ...
        >
          ...
          >
            {(item) => (
              <PublishedAppsDashboardTableRow item={item} key={item.name} />
            )}
          </DashboardTable>
