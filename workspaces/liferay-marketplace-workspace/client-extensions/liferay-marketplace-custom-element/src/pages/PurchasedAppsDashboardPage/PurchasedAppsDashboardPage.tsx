...
export function PurchasedAppsDashboardPage() {
  ...
  const [purchasedAppTable, setPurchasedAppTable] = useState<PurchasedAppTable>(
          ...
          filteredAppOrders.map(async (order) => {
            ...
            if (attachments) {
              orderThumbnail = await (async () => {
                const promises = attachments.map(async (currentAttachment) => {
            ...
            return {
              ...
              type: placeOrderItem.subscription ? 'Subscription' : 'Perpetual',
    ...
    <div className="purchased-apps-dashboard-page-container">
      ...
      {!loading && selectedNavigationItem === 'My Apps' && (
        ...
        >
          ...
          >
            {(item) => (
              <PurchasedAppsDashboardTableRow item={item} key={item.name} />
            )}
          </DashboardTable>
