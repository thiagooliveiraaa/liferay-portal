 ...
 * @generated
 */
@Component(service = FVSEntryPersistence.class)
public class FVSEntryPersistenceImpl
	...
	public void cacheResult(List<FVSEntry> fvsEntries) {
		if ((_valueObjectFinderCacheListThreshold == 0) ||
			((_valueObjectFinderCacheListThreshold > 0) &&
			 (fvsEntries.size() > _valueObjectFinderCacheListThreshold))) {
	...
	public FVSEntry updateImpl(FVSEntry fvsEntry) {
		...
		if (isNew && (fvsEntry.getCreateDate() == null)) {
			if (serviceContext == null) {
				fvsEntry.setCreateDate(date);
			}
			else {
				fvsEntry.setCreateDate(serviceContext.getCreateDate(date));
			}
		}

		if (!fvsEntryModelImpl.hasSetModifiedDate()) {
			if (serviceContext == null) {
				fvsEntry.setModifiedDate(date);
			}
			else {
				fvsEntry.setModifiedDate(serviceContext.getModifiedDate(date));
			}
		}
	...
	public void activate() {
		...
		FVSEntryUtil.setPersistence(this);
	}

	@Deactivate
	public void deactivate() {
		FVSEntryUtil.setPersistence(null);
