...
public class CommerceOrderNotePersistenceImpl
		...
		if (useFinderCache) {
			...
			if ((list != null) && !list.isEmpty()) {
				for (CommerceOrderNote commerceOrderNote : list) {
					if (!uuid.equals(commerceOrderNote.getUuid())) {
						list = null;
	...
	public CommerceOrderNote[] findByUuid_PrevAndNext(
		...
		try {
			...
			array[0] = getByUuid_PrevAndNext(
				session, commerceOrderNote, uuid, orderByComparator, true);

			array[1] = commerceOrderNote;

			array[2] = getByUuid_PrevAndNext(
				session, commerceOrderNote, uuid, orderByComparator, false);
	...
	protected CommerceOrderNote getByUuid_PrevAndNext(
		Session session, CommerceOrderNote commerceOrderNote, String uuid,
		OrderByComparator<CommerceOrderNote> orderByComparator,
		boolean previous) {
	...
	public CommerceOrderNote fetchByUUID_G(
		...
		if (result instanceof CommerceOrderNote) {
			...
			if (!Objects.equals(uuid, commerceOrderNote.getUuid()) ||
	...
	private static final String _FINDER_COLUMN_UUID_G_UUID_2 =
		"commerceOrderNote.uuid = ? AND ";

	private static final String _FINDER_COLUMN_UUID_G_UUID_3 =
		"(commerceOrderNote.uuid IS NULL OR commerceOrderNote.uuid = '') AND ";

	private static final String _FINDER_COLUMN_UUID_G_GROUPID_2 =
		"commerceOrderNote.groupId = ?";
	...
	public CommerceOrderNote[] findByUuid_C_PrevAndNext(
		...
		try {
			...
			array[0] = getByUuid_C_PrevAndNext(
				session, commerceOrderNote, uuid, companyId, orderByComparator,
				true);
	...
	protected CommerceOrderNote getByUuid_C_PrevAndNext(
		Session session, CommerceOrderNote commerceOrderNote, String uuid,
		long companyId, OrderByComparator<CommerceOrderNote> orderByComparator,
		boolean previous) {
