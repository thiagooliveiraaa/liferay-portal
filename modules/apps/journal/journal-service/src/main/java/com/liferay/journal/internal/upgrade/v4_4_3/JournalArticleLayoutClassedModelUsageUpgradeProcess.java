...
public class JournalArticleLayoutClassedModelUsageUpgradeProcess
	...
	private void _addAssetPublisherPortletPreferencesLayoutClassedModelUsages(
		...
		String sql = StringBundler.concat(
			...
			"PortletPreferences.portletId from AssetEntry AssetEntry, ",
			"PortletPreferences PortletPreferences, (select ",
			"SUBSTR(smallValue, INSTR(smallValue, '<asset-entry-uuid>') + ",
			"LENGTH('<asset-entry-uuid>'), INSTR(smallValue, ",
			"'</asset-entry-uuid>') - (INSTR(smallValue, ",
			"'<asset-entry-uuid>') + LENGTH('<asset-entry-uuid>'))) uuid, ",
			"portletPreferencesId from PortletPreferenceValue where name = ",
			"'assetEntryXml') temp where ",
			"PortletPreferences.portletPreferencesId = ",
			"temp.portletPreferencesId and PortletPreferences.ownerId = ",
			...
			" and AssetEntry.classUuid = temp.uuid  and AssetEntry.visible = ",
			"[$TRUE$] and PortletPreferences.portletPreferencesId in (select ",
			"portletPreferencesId from PortletPreferenceValue where name = ",
			"'selectionStyle' and smallValue = 'manual') and not exists ",
